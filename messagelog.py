@nightyScript(
    ...
    usage=(
        ...
        "• `(prefix)blacklistrole <add/remove/list> <@role>` → Manage role blacklist for message logging.\n\n"
...
def messageLogger():
    ...
    def initialize_files():
        ...
        if not CONFIG_FILE.exists():
            default_config = {
                "main_webhook": "https://discord.com/xxx/xxxxxxx/xxxxxxxxxxxxxxxxxxx/xxxxxxxxxxxxxxxxxxx",
                "image_dump_webhook": "https://discord.com/xxx/xxxxxxx/xxxxxxxxxxxxxxxxxxx/xxxxxxxxxxxxxxxxxxx",
                "custom_avatar_url": "https://xxxx.xxx/xxxxxxxxxxxxxxxxxxx/xxxxxxxxxxxxxxxxxxx.png",
    ...
    def summarize_embeds(embeds):
        """Return a compact text summary for a list of Discord embeds."""
        try:
            if not embeds:
                return "No Embeds"
            ...
            for idx, e in enumerate(embeds, start=1):
                ...
                etype = d.get("type") or getattr(e, "type", None) or "rich"
                title = d.get("title") or getattr(e, "title", None) or ""
                url = d.get("url") or getattr(e, "url", None) or ""
                desc = d.get("description") or getattr(e, "description", None) or ""

                # Trim overly long description
                if desc and len(desc) > 180:
                    desc = desc[:177] + "..."

                snippet = []
                snippet.append(f"[{idx}] {etype}")
                if title:
                    snippet.append(f"Title: {title}")
                if url:
                    snippet.append(f"URL: {url}")
                if desc:
                    snippet.append(f"Desc: {desc}")

                parts.append(" | ".join(snippet))

            text = "\n".join(parts)
            # Discord field value limits safety
            if len(text) > 1000:
                text = text[:997] + "..."
            return text or "No Embeds"
        except Exception:
            return "Embeds present (summary unavailable)"
    ...
    async def log_deleted_message(message):
        ...
        embeds_text = summarize_embeds(message.embeds)
        ...
        embed_data = {
            ...
            "fields": [
                ...
                {"name": "🧩 Embeds", "value": embeds_text, "inline": False},
    ...
    async def log_bulk_deleted_messages(messages):
        ...
        message_log = "\n".join(
            [
                f"**{msg.author.name}:** `{msg.content or '*No Content*'}`" +
                (f" • [Embeds: {len(msg.embeds)}]" if getattr(msg, "embeds", None) else "")
    ...
    async def log_edited_message(before, after):
        ...
        embed_data = {
            ...
            "fields": [
                ...
                {"name": "🎥 Videos", "value": video_text, "inline": False},
                {"name": "🎵 Audios", "value": audio_text, "inline": False},
                {"name": "🧩 Embeds (Before)", "value": before_embeds_text, "inline": False},
                {"name": "🧩 Embeds (After)", "value": after_embeds_text, "inline": False},
